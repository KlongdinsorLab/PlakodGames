...
export default class OtpScene extends Phaser.Scene {
    ...
    create() {
        ...
        const title = i18n.createTranslatedText(this, width/2, 3* MARGIN, "otp_title", undefined, {fontSize:LARGE_FONT_SIZE}).setOrigin(0.5, 0)
        i18n.createTranslatedText(this, width/2, title.y + 2* MARGIN, "otp_description", undefined, {fontSize:MEDIUM_FONT_SIZE}).setOrigin(0.5, 0)

        const element = this.add.dom(480, height/2).createFromCache('otpForm').setScale(1.5)
        ...
        element.on('input', (event: DOMEvent<HTMLInputElement>) => {
					...
					const nextIndex =
						event.target.value === '' ? otpIndex - 1 : otpIndex + 1
					const next = <HTMLInputElement>(
						document.getElementById(`otp${nextIndex}`)
					)
					try {
						next.focus()
